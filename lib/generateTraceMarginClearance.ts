...
/**
 * Generates trace margin clearance areas for a given layer
 * Computes the area between inner (normal) and outer (with margin) trace geometries
 * and outputs them as filled shapes with crosshatch pattern
 */
export const generateTraceMarginClearance = (
  ctx: ConvertContext,
  layer: "top" | "bottom",
) => {
  const {
  ...
  // Early return if trace margin is not enabled
  ...
  for (const net of Object.keys(connMap.netMap)) {
    ...
    try {
      ...
      console.warn(
        `Failed to generate trace margin clearance for net ${net} on ${layer} layer:`,
        error,
      )
